// Long-form Word rendering of methodology.md, the accompanying document to
...
// each pipeline stage, in this project's own palette (deep teal / sage).
  ...
  sections: [
    // ---------------- COVER ----------------
    {
      ...
      children: [
        ...
        new Paragraph({ spacing: { before: 60, after: 300 }, children: [new TextRun({ text: "A pre-registered proof-of-concept testing whether public search interest and academic publishing activity amplify the case that women’s brain health is economically material", size: 26, color: SAGE })] }),
    ...
    // ---------------- MAIN BODY ----------------
    {
      ...
      children: [
        body("This is the canonical methodology document for this proposal. It sets out a three-layer architecture and pipeline structure for anticipatory policy intelligence from open digital signals, applied here to women’s brain health.", { italics: true, color: GREY }),
        ...
        body("The pipeline runs in five conceptual stages, each with a verified script order. Scripts and their inputs/outputs are described below as they will work once run; at pre-registration time, none had been executed yet."),
        ...
        dataTable(
          ...
          [
            ...
            ["mesh_alignment.py", "Matches each candidate to the closest MeSH descriptor by text similarity. A zero-similarity result is reported as no match, not an arbitrary nearest label, fixed before this stage is run", "Prevents a known text-similarity failure mode from ever entering the results"],
        ...
        dataTable(
          ...
          [
            ...
            ["generate_report_figures.py", "Generates the figures for the briefing note", "Chart generation for the final report"],
        ...
        bullet("Construct validity: search growth and publication growth may reflect attention rather than genuine emerging need."),
        ...
        body("Output at that stage: not a full formatted briefing note. A short proposal (see the accompanying evidence document) reporting the emergence score per topic, without yet claiming a recognition gap; the MeSH layer (Stage 4) would follow only if the emergence results justify the additional step."),
